namespace FreeControls
{
                                      ...
                                      ICloneable
    {
        ...
        {
            int d, m, y;
            y = int.Parse(dateString.Substring(0, 4));
            m = int.Parse(dateString.Substring(5, 2));
            d = int.Parse(dateString.Substring(8, 2));
            PersianDate pd = new PersianDate(y, m, d);
            return pd;
        }

        public static PersianDate Parsewh(string dateString)
        {
            int day, month, year,h,m,s;
            year = int.Parse(dateString.Substring(0, 4));
            month = int.Parse(dateString.Substring(5, 2));
            day = int.Parse(dateString.Substring(8, 2));

            h = int.Parse(dateString.Substring(10, 2));
            m = int.Parse(dateString.Substring(13, 2));
            s = 0;
            PersianDate pd = new PersianDate(year, month, day, h, m, s);
            return pd;
        }
